...
var itemsearch = function(item) {
	...
	var sentences = tokenizer.sentences(item, {});
		...
		qualifiedFind: function( regex, pre, post, callback ) {
			...
			sentences.map( function( value, index ) {
				...
				if (candidates) {
					...
					var sentenceParts = value.split(splitExp);
					// split the sentence into an array containing the part before the match and the part after


					if (sentenceParts[0] == ' ') {sentenceParts.pop();}
				
					var l = sentenceParts.length;

					sentenceParts.map(function (v,i,a) {

						var lastpart = v;
					
						var nextpart = a[i+1] ? a[i+1] : null;

						// if we're searching for the next part 
						if ( (i = l) && (!nextpart) ) { 
